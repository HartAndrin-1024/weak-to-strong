...
def main(model_sizes: Union[List[str], str], prop_of_gt: str='0', hard_questions_only='', upweight_gt_factor='1' ,med_questions_only='',weighted_auto='1.0',online_correction='',curriculum='',upweight_med_quest='',**kwargs):
    ...
    upweight_med_quest=str(upweight_med_quest)
    ...
    for i in range(len(model_sizes)):
        for j in range(i, len(model_sizes)):
            ...
            subprocess.run(
                basic_args
                + ["--weak_model_size", weak_model_size, "--model_size", strong_model_size, "--prop_of_gt", prop_of_gt, "--hard_questions_only",hard_questions_only, "--upweight_gt_factor", upweight_gt_factor, "--med_questions_only", med_questions_only, "--weighted_auto", weighted_auto,"--online_correction",online_correction, '--curriculum', curriculum, '--upweight_med_quest',upweight_med_quest],
                check=True,
            )
